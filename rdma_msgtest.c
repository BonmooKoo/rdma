...
static int client_remote_memory_ops() 
{
	...
	for(int i=0;i<time;i++){
		...
		client_send_wr.wr.rdma.remote_addr = server_metadata_attr.address+msg_size*i;
		/* Now we post it */
	...
	for(int j=0;j<time;j++){
		client_send_sge.addr = (uint64_t) client_dst_mr->addr+msg_size*j;
		client_send_sge.length = msg_size;
		...
		/* we have to tell server side info for RDMA */
		client_send_wr.wr.rdma.rkey = server_metadata_attr.stag.remote_stag;
		client_send_wr.wr.rdma.remote_addr = server_metadata_attr.address+msg_size*j;
		/* Now we post it */
		struct timespec t1,t2;
		clock_gettime(CLOCK_REALTIME,&t1);
		ret = ibv_post_send(client_qp, 
		...
		/* at this point we are expecting 1 completion for the write */
		ret = process_work_completion_events(io_completion_channel, 
				&wc, 1);
		if(ret !=1) {
			rdma_error("We failed to get 1 work completions , ret = %d \n",
...
void usage() {
	printf("Usage:\n");
	printf("rdma_client: [-a <server_addr>] [-p <server_port>] -s msgsize\n");
